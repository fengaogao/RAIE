...
class SASRecFinetuneWindowDataset(Dataset):
    """Windowed finetuning samples: prompt -> target at the final position."""
...
class EvalWindowDataset(Dataset):
    """Evaluation samples: (seq, tgt_idx, ctx_list)."""
...
# ------------------------------
# SASRec (same logic as the original implementation, reorganized)
# ------------------------------
...
@torch.no_grad()
def _adapter_logits_sasrec(model: nn.Module, seq: torch.Tensor, ctx_list: List[np.ndarray], n_items: int,
                           adapter_name: str, device, exclude_seen: bool = True) -> torch.Tensor:
    ...
    last_pos = lengths - 1
    h_t = h[torch.arange(h.size(0), device=device), last_pos]
    logits = model.score(h_t)
    scores = logits.clone()
    if exclude_seen:
        for i, ctx_np in enumerate(ctx_list):
            seen = set(int(x) for x in ctx_np.tolist() if x > 0)
            seen.discard(int((seq[i, last_pos[i]].item()) if last_pos[i] < seq.size(1) else 0))
            if seen:
                seen0 = torch.tensor([s-1 for s in seen], device=scores.device, dtype=torch.long)
                scores[i].index_fill_(0, seen0, float('-inf'))
    return scores
...
@torch.no_grad()
def evaluate_lsat(model: nn.Module, loader: DataLoader, n_items: int, topk_list: List[int], device,
                  long_adapter: str, short_adapter: str, alpha: float = 0.5, exclude_seen: bool = True) -> Dict[str, float]:
    ...
    for batch in tqdm(loader, desc="Eval-LSAT", leave=False):
        ...
        tgt_idx = batch['tgt'].to(device)
        ctx_list = batch['ctx']
        logits_long = _adapter_logits_sasrec(model, seq, ctx_list, n_items, long_adapter, device, exclude_seen=exclude_seen)
        logits_short = _adapter_logits_sasrec(model, seq, ctx_list, n_items, short_adapter, device, exclude_seen=exclude_seen)
        scores = alpha * logits_long + (1.0 - alpha) * logits_short
        top_idx = torch.argsort(scores, dim=1, descending=True)
        tgt0 = (tgt_idx - 1).unsqueeze(1)
        pos = (top_idx == tgt0).nonzero(as_tuple=False)
        B = seq.size(0)
...
class _MoLEOutput:
    def __init__(self, hidden: torch.Tensor, loss: Optional[torch.Tensor], gate: torch.Tensor):
        self.hidden = hidden
        self.loss = loss
        self.gate = gate
...
class MoLEMixture(nn.Module):
    """Mixture-of-LoRA-Experts for SASRec."""

    def __init__(self, base_model: SASRec, adapter_names: List[str], n_items: int,
                 gating_hidden: int = 128, temperature: float = 0.7, balance_coef: float = 0.01):
        super().__init__()
        ...
        emb_dim = self.model.item_emb.embedding_dim
        self.gate = nn.Sequential(
            nn.Linear(emb_dim, gating_hidden),
            nn.Tanh(),
    ...
    def forward(self, seq_ids: torch.Tensor, pos: Optional[torch.Tensor] = None,
                neg: Optional[torch.Tensor] = None, return_loss: bool = False):
        emb = self.model.item_emb(seq_ids)
        mask = (seq_ids != 0).unsqueeze(-1)
        ...
        for name in self.adapter_names:
            ...
            hidden_list.append(self.model(seq_ids))
        hidden_stack = torch.stack(hidden_list, dim=1)  # [B, E, L, D]
        mix_hidden = torch.einsum('be,beLD->bLD', gate_weights, hidden_stack)

        loss = None
        if return_loss and (pos is not None) and (neg is not None):
            pos_emb = self.item_emb(pos)
            neg_emb = self.item_emb(neg)
            pos_logits = (mix_hidden * pos_emb).sum(dim=-1)
            neg_logits = (mix_hidden * neg_emb).sum(dim=-1)
            mask_pos = (pos > 0).float()
            loss_pos = F.binary_cross_entropy_with_logits(pos_logits, torch.ones_like(pos_logits), reduction='none')
            loss_neg = F.binary_cross_entropy_with_logits(neg_logits, torch.zeros_like(neg_logits), reduction='none')
            base_loss = ((loss_pos + loss_neg) * mask_pos).sum() / mask_pos.sum().clamp_min(1.0)

            balance_loss = torch.tensor(0.0, device=seq_ids.device)
        ...
        out = _MoLEOutput(hidden=mix_hidden, loss=loss, gate=gate_weights)
        return out if return_loss else out.hidden

    def score(self, h_t: torch.Tensor) -> torch.Tensor:
        return self.model.score(h_t)
    ...
    *,
    ...
    for step, batch in enumerate(tqdm(finetune_loader, desc="Finetune(F)-LoRA", leave=False), start=1):
        ...
        if ('replay' in plugin) and (replay_buf is not None) and (replay_ratio > 0.0):
            ...
            rpos = rb['pos'].to(device)
            rneg = rb['neg'].to(device)
            hr = model(rseq)
            rpos_emb = model.item_emb(rpos)
            rneg_emb = model.item_emb(rneg)
            rpos_logits = (hr * rpos_emb).sum(dim=-1)
            rneg_logits = (hr * rneg_emb).sum(dim=-1)
            rmask = (rpos > 0).float()
...
def finetune_one_epoch_mole(
    ...
    *,
    ...
    for step, batch in enumerate(tqdm(finetune_loader, desc="Finetune(F)-MoLE", leave=False), start=1):
        ...
        optimizer.zero_grad(set_to_none=True)
        out = mole_model(seq, pos=pos, neg=neg, return_loss=True)
        loss = out.loss
        loss.backward()
        if grad_clip and grad_clip > 0: clip_grad_norm_(mole_model.parameters(), grad_clip)
        optimizer.step()
...
# =========================================================
# ===== RAIE (same SASRec-RAIE logic, refactored layout) =====
# =========================================================
...
def _clone_lora_adapter_weights(peft_model: nn.Module, src_name: str, dst_name: str):
    for m in peft_model.modules():
        hasA = hasattr(m, "lora_A")
        hasB = hasattr(m, "lora_B")
...
class RegionBankSASRec:
    ...
    def _maybe_merge_by_angle(self, angle_thr=np.deg2rad(12), max_pairs=1):
        ...
        for _, i, j in pairs:
            ...
            self.n[i] = self.n[i] + self.n[j]
            self.S = np.delete(self.S, j, axis=0)
            self.n = np.delete(self.n, j, axis=0)
            self.C = np.delete(self.C, j, axis=0)
            if self.C0 is not None:
                self.C0 = np.delete(self.C0, j, axis=0)
            if self.R is not None:
                self.R = np.delete(self.R, j, axis=0)
            if self.sig is not None:
                self.sig = np.delete(self.sig, j, axis=0)
            if self.kappa is not None:
                self.kappa = np.delete(self.kappa, j, axis=0)
            if self.pi is not None:
                self.pi = np.delete(self.pi, j, axis=0)
            merged += 1
    ...
    def map_finetune(self, finetune_prompts: Dataset, pool_min=200, bic_gain=1e4):
        ...
        if len(add_pool) >= pool_min:
            ...
            # BIC acceptance check (same logic as original)
            if len(centers)>0:
                # Approximate BIC: compare max-likelihood gain with complexity penalty.
    ...
    def route_and_eval(self, test_rows: List[dict], id2idx: Dict[int,int],
        ...
        for k, idxs in idx_by_k.items():
            ...
            m["cluster_id"] = int(k)
            m["num_samples"] = len(sub)
            all_stats.append(m)
        return all_stats

# =========================================================
# Main (modes: base / lora / lora_replay / lora_lwf / lsat / raie / mole)
# =========================================================
def main():
    ...
    # MoLE
    parser.add_argument('--mole_num_experts', type=int, default=3, help='Number of LoRA experts for MoLE')
    parser.add_argument('--mole_gating_hidden', type=int, default=128, help='Hidden size of MoLE gating network')
    parser.add_argument('--mole_temp', type=float, default=0.7, help='Temperature for MoLE gating softmax')
    parser.add_argument('--mole_balance', type=float, default=0.01, help='KL weight to balance MoLE expert usage')
    ...
    parser.add_argument('--orig_mix_ratio', type=float, default=0.7)
    ...
    if args.mode in ('lora','lora_replay','lora_lwf','lsat','raie','mole') and (not PEFT_AVAILABLE):
    ...
    original_path = os.path.join(args.data_dir, "original.jsonl")
    if not os.path.exists(original_path): raise FileNotFoundError(original_path)
    original_rows = read_jsonl(original_path)
    original_ds = EvalWindowDataset(original_rows, id2idx, maxlen=args.maxlen)
    original_loader = DataLoader(original_ds, batch_size=512, shuffle=False, num_workers=0, collate_fn=eval_collate)
    ...
    if args.mode == 'base':
        original_metrics = evaluate_windows(base_model, original_loader, n_items, list(topk_tuple), device)
        print("[original][base]", ' '.join([f"{k}:{v:.6f}" for k,v in sorted(original_metrics.items())]))
    ...
    # ===== LoRA wrapper (lora / lora_replay / lora_lwf / lsat / raie / mole) =====
        ...
        if (not os.path.exists(finetune_path)):
            ...
            if args.mode == 'lsat':
                ...
                for ep in range(1, args.lsat_long_epochs + 1):
                    tr = train_one_epoch(lora_model, train_loader, device, long_optim, grad_clip=args.grad_clip)
                ...
                original_metrics = evaluate_lsat(
                    lora_model, original_loader, n_items, list(topk_tuple), device,
                    long_adapter=long_adapter, short_adapter=short_adapter,
                    alpha=args.lsat_alpha, exclude_seen=True
                )
                tag = args.mode
                print(f"[original][{tag}]", ' '.join([f"{k}:{v:.6f}" for k, v in sorted(original_metrics.items())]))

                metrics = evaluate_lsat(
                    lora_model, eval_loader, n_items, list(topk_tuple), device,
                    long_adapter=long_adapter, short_adapter=short_adapter,
                    alpha=args.lsat_alpha, exclude_seen=True
                )
                tag = args.mode
            ...
            # Selective unfreezing of item embedding rows + anchor.
            emb = lora_model.get_input_embeddings() if hasattr(lora_model, "get_input_embeddings") else lora_model.item_emb
            E0 = emb.weight.detach().clone()
            tune_ids = set()
            for r in rows_f:
    ...
    if args.mode == 'mole':
        ...
        for i in range(num_exp):
            ...
            if hasattr(lora_model, "train_adapter"):
                try: lora_model.train_adapter(name)
                except Exception: pass

        mole_model = MoLEMixture(
            base_model=lora_model, adapter_names=adapter_names, n_items=n_items,
            gating_hidden=args.mole_gating_hidden, temperature=args.mole_temp, balance_coef=args.mole_balance
        ).to(device)

        no_decay = ['bias', 'LayerNorm.weight']
        params = [
            {"params": [p for n, p in mole_model.named_parameters() if p.requires_grad and not any(nd in n for nd in no_decay)],
             "weight_decay": args.weight_decay},
            {"params": [p for n, p in mole_model.named_parameters() if p.requires_grad and any(nd in n for nd in no_decay)],
             "weight_decay": 0.0},
        ]
        ...
        for ep in range(1, args.finetune_epochs + 1):
            ft_loss = finetune_one_epoch_mole(
                mole_model, ft_loader, device, ft_optim,
                grad_clip=args.grad_clip, log_every=100
            )
            print(f"[MoLE][Epoch {ep}] loss={ft_loss:.4f}")

        original_metrics = evaluate_windows(mole_model, original_loader, n_items, list(topk_tuple), device)
        print(f"[original][mole]", ' '.join([f"{k}:{v:.6f}" for k, v in sorted(original_metrics.items())]))
        metrics = evaluate_windows(mole_model, eval_loader, n_items, list(topk_tuple), device)
        print(f"[Test][mole]", ' '.join([f"{k}:{v:.6f}" for k, v in sorted(metrics.items())]))
    ...
    rows_f = read_jsonl(finetune_path)
    # Routing samples: prefer original_stride1.jsonl otherwise build from stream.
