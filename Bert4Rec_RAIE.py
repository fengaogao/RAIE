...
class MoLEMixture(nn.Module):
    ...
    We build multiple LoRA adapters on top of the shared Bert backbone and
    train a lightweight gating network that selects experts conditioned on
    the current sequence (pooling input embeddings). The design follows the
    MoLE intuition from https://arxiv.org/pdf/2404.13628 without depending on
    external code.
    """
    ...
    def forward(self, input_ids, attention_mask, labels=None):
        ...
        logits_stack = torch.stack(logits_list, dim=1)  # [B, E, L, V]
        ...
        if labels is not None:
            final_pos = attention_mask.sum(dim=1) - 1
            rows = torch.arange(input_ids.size(0), device=input_ids.device)
            logits_final = mix_logits[rows, final_pos, :]

            V = logits_final.size(-1)
            mask_vec = torch.full((V,), float('-inf'), device=input_ids.device)
            mask_vec[self.item_token_ids] = 0.0
            logits_final = logits_final + mask_vec
        ...
        return _Output(logits=mix_logits, loss=loss, gate=gate_weights)
...
# ------------------------------
# F-stage: LoRA finetuning (shared by modes 2-4)
# ------------------------------
...
# =========================================================
# ===== RAIE section: preserve the core implementation flow =====
# =========================================================
...
class RegionBank:
    ...
    def _scores_post(self, x: np.ndarray):
        ...
        # K=1: skip top-2 and return cluster 0.
        if scores.shape[0] == 1:
            z = scores - scores.max()
    ...
    def map_finetune(self, finetune_ds, pool_min=200, bic_gain=1e4):
        """
        RAIE-off: disable region edits (no expansion or additions). Map F-stage
        samples to the nearest existing cluster and update S/n for re-estimation.
        """
        ...
        # K=1: all samples go to cluster 0.
        if self.C.shape[0] == 1:
            buckets = defaultdict(list)
            for i in range(Xn.shape[0]):
                buckets[0].append(i)
        ...
        # K>=2: assign to the best posterior cluster, no expansion.
        buckets = defaultdict(list)
        for i in range(Xn.shape[0]):
            x = Xn[i]
            k1, *_ = self._scores_post(x)
            buckets[int(k1)].append(i)
            self.S[int(k1)] += x
            self.n[int(k1)] += 1.0

        # Recompute parameters; keep radius/std unchanged.
        self._recompute_mu_kappa_pi()
        np.savez(os.path.join(self.out_dir, "raie_regions_after_map.npz"),
                 centroids=self.C, radii=self.R, sig=self.sig,
                 kappa=self.kappa, pi=self.pi, S=self.S, n=self.n)
...
def main():
    ...
    # MoLE args
    parser.add_argument('--mole_num_experts', type=int, default=3,
                        help='Number of LoRA experts to mix (including the default adapter)')
    parser.add_argument('--mole_gating_hidden', type=int, default=128,
                        help='Hidden size of the gating MLP')
    parser.add_argument('--mole_temp', type=float, default=0.7,
                        help='Softmax temperature for gating')
    parser.add_argument('--mole_balance', type=float, default=0.01,
                        help='KL regularization weight encouraging balanced expert usage')

    # RAIE args (aligned with the original implementation)
    ...
    original_path_tgt = os.path.join(args.data_dir, 'original.jsonl')
    original_path_alt = os.path.join(args.data_dir, 'original.jsonl')
    test_path_tgt = os.path.join(args.data_dir, 'test.jsonl')
    test_path_alt = os.path.join(args.data_dir, 'test.jsonl')
    if not os.path.exists(train_path):
        raise FileNotFoundError('Missing original_stream.jsonl in data_dir')

    # Test path prioritizes test.jsonl.
    ...
    orig_ds = NextItemDataset(original_path_tgt)
    test_ds = NextItemDataset(test_path)

    # RAIE expects original_stride1.jsonl.
    ...
    if args.objective == 'cloze':
        ...
        # For the "next" objective, this keeps cloze collator to preserve behavior.
        train_collator = ClozeTrainCollator(token2id, args.max_len, pad_id, mask_id, cls_id, args.mask_prob)
    ...
    orig_loader = DataLoader(orig_ds, batch_size=args.batch_size, shuffle=False, num_workers=2,
    ...
    if args.mode == 'base':
        ...
        ori_metrics = evaluate(base_model, orig_loader, device, item_token_ids, topk=topk_tuple)
        metrics = evaluate(base_model, test_loader, device, item_token_ids, topk=topk_tuple)
        print("[Original][base]", ' '.join([f"{k}:{v:.4f}" for k, v in sorted(ori_metrics.items())]))
    ...
    # ---------- Shared: LoRA wrapping (modes 2-4 and RAIE) ----------
    ...
    if args.mode in ('lora','lora_replay','lora_lwf','lsat'):
        if (finetune_ds is None) or (len(finetune_ds) == 0):
            ...
            if args.mode == 'lsat':
                ...
                orig_metrics = evaluate_lsat(
                    lora_model, orig_loader, device, item_token_ids,
                ...
                tag = args.mode
                print(f"[Test][{tag}]", ' '.join([f"{k}:{v:.4f}" for k, v in sorted(orig_metrics.items())]))
            ...
            for ep in range(1, args.finetune_epochs + 1):
                ft_loss = finetune_one_epoch_lora(
                    ...
                    ewc_state=None,  # Keep the interface but disabled.
                    teacher=teacher,
        ...
        orig_metrics = evaluate(eval_model, orig_loader, device, item_token_ids, topk=topk_tuple)
    ...
    # ---------- Mode: MoLE (Mixture-of-LoRA-Experts) ----------
    if args.mode == 'mole':
        if (finetune_ds is None) or (len(finetune_ds) == 0):
        ...
        orig_metrics = evaluate(mole_model, orig_loader, device, item_token_ids, topk=topk_tuple)
        print("[Test][MoLE]", ' '.join([f"{k}:{v:.4f}" for k, v in sorted(orig_metrics.items())]))
        metrics = evaluate(mole_model, test_loader, device, item_token_ids, topk=topk_tuple)
    ...
    # Before F-stage: snapshot baseline embeddings, tune only F-stage target items.
    ...
    # F-stage for RAIE: uses finetune_one_epoch_bert.
    ...
    orig_stats = bank.route_and_eval(orig_ds, eval_collator, item_token_ids, _eval_fn_for_raie,
                                k_list=topk_tuple)
    ...
    orig_w = orig_df["num_samples"]
    orig_W = max(1, orig_w.sum())
    global_orig_metrics = {m: float((orig_df[m] * orig_w).sum() / orig_W) for m in orig_df.columns if m.startswith(("Recall@", "NDCG@"))}
    global_metrics = {m: float((df[m] * w).sum() / W) for m in df.columns if m.startswith(("Recall@", "NDCG@"))}
    print("[RAIE][Global][original]", global_orig_metrics)
    print("[RAIE][Global][Test]", global_metrics)
