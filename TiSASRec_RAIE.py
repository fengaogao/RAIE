...
class SeqDataset(Dataset):
    """Read prompt/target data from finetune.jsonl or test.jsonl."""
...
class StreamPairDataset(Dataset):
    """
    O-stage training dataset from original_stream.jsonl (one full user sequence per line),
    using a sliding window to generate (prompt -> target) samples.
    Example line:
      {"user_id": ..., "items": "<item_a> <item_b> ...", "timestamps": "t_a t_b ..."}
    """
...
class Collator:
    ...
    def __call__(self, batch: List[Example]):
        ...
        for i, ex in enumerate(batch):
            ...
            labels[i] = lab if lab in self.item_id_set else -100
...
class MoLEMixture(nn.Module):
    """Mixture-of-LoRA-Experts for TiSASRec."""
    ...
    def __init__(
        ...
        self.balance_coef = max(0.0, balance_coef)

        emb_layer = self._get_input_emb()
        dim = emb_layer.embedding_dim
        self.gate = nn.Sequential(
            nn.Linear(dim, gating_hidden),
            nn.Tanh(),
            nn.Linear(gating_hidden, len(self.adapter_names)),
        )
    ...
    def _get_input_emb(self):
        ...
        if hasattr(self.model, "item_emb"):
            return self.model.item_emb
        raise RuntimeError("Model does not expose input embeddings for MoLE gating.")

    def forward(self, seq_ids, attn_mask, t_buckets, seqlen, labels=None):
        with torch.set_grad_enabled(self.training):
            emb_layer = self._get_input_emb()
            emb = emb_layer(seq_ids)
            mask = (seq_ids != self.pad_id).unsqueeze(-1).float()
        ...
        for name in self.adapter_names:
            if hasattr(self.model, "set_adapter"):
                try:
                    ...
                    pass
            h = self.model(seq_ids=seq_ids, attn_mask=attn_mask, t_buckets=t_buckets, pad_id=self.pad_id)
            logits = self.model.predict_last(h, seqlen)
            logits_list.append(logits)

        logits_stack = torch.stack(logits_list, dim=1)  # [B, E, V]
        mix_logits = torch.einsum('be,bev->bv', gate_weights, logits_stack)

        loss = None
        if labels is not None:
            logits_final = _masked_item_logits(mix_logits, self.item_token_ids)
            base_loss = F.cross_entropy(logits_final, labels)
            balance_loss = torch.tensor(0.0, device=logits_final.device)
            if self.balance_coef > 0:
                avg_gate = gate_weights.mean(dim=0)
                uniform = torch.full_like(avg_gate, 1.0 / len(self.adapter_names))
                balance_loss = self.balance_coef * F.kl_div(
                    (avg_gate + 1e-8).log(), uniform, reduction='batchmean'
                )
...
def finetune_one_epoch_mole(
    ...
    for step, batch in enumerate(tqdm(finetune_loader, desc="Finetune(F)-MoLE", leave=False), start=1):
        seq = batch['seq'].to(device)
        t_bk = batch['t_buckets'].to(device)
        attn_mask = batch['attn_mask'].to(device)
        labels = batch['labels'].to(device)
        ...
        out = mole_model(seq_ids=seq, attn_mask=attn_mask, t_buckets=t_bk, seqlen=seqlen, labels=labels)
        loss = out.loss
        loss.backward()
        if grad_clip and grad_clip > 0:
            clip_grad_norm_(mole_model.parameters(), grad_clip)
        optimizer.step()
        ...
        if (log_every is not None) and (log_every > 0) and (step % log_every == 0):
            print(f"[MoLE][step {step}] loss={loss.item():.4f}")
...
def finetune_one_epoch(model: nn.Module, ft_loader: DataLoader, optimizer, scheduler,
    ...
    """
    Supports TiSASRec or PeftModel(TiSASRec) with a shared interface.
    """
    model.train()
    losses = []
    for batch in tqdm(ft_loader, desc='Finetune(F)', leave=False):
        ...
        # EWC (disabled in five-mode setup, kept for compatibility)
...
def evaluate(model: nn.Module, loader: DataLoader, device, pad_id, item_token_ids, topk):
    model.eval()
    sumd = {k: {'hit': 0.0, 'dcg': 0.0} for k in topk}
    N = 0
...
def _adapter_final_logits_tisas(
    model: nn.Module,
    adapter_name: str,
    seq: torch.Tensor,
    ...
    if hasattr(model, "set_adapter"):
        try:
            ...
            pass
    h = model(seq_ids=seq, attn_mask=attn_mask, t_buckets=t_buckets, pad_id=pad_id)
    logits = model.predict_last(h, seqlen)
    return _masked_item_logits(logits, item_token_ids)


@torch.no_grad()
def evaluate_lsat_tisas(
    model: nn.Module,
    loader: DataLoader,
    device: torch.device,
    pad_id: int,
    ...
    for batch in tqdm(loader, desc='Eval-LSAT', leave=False):
        ...
        logits_long = _adapter_final_logits_tisas(model, long_adapter, seq, attn_mask, t_bk, seqlen, pad_id, item_token_ids)
        logits_short = _adapter_final_logits_tisas(model, short_adapter, seq, attn_mask, t_bk, seqlen, pad_id, item_token_ids)
        logits_final = alpha * logits_long + (1.0 - alpha) * logits_short

        sums, bsz = recall_ndcg_sums(logits_final, labels, item_token_ids, topk=tuple(topk))
        for K in sums:
            sumd[K]['hit'] += sums[K]['hit']
            sumd[K]['dcg'] += sums[K]['dcg']
        N += bsz
...
def evaluate_mole_tisas(
    ...
    sumd = {k: {'hit': 0.0, 'dcg': 0.0} for k in topk}
    N = 0
    for batch in tqdm(loader, desc='Eval-MoLE', leave=False):
        seq = batch['seq'].to(device)
        t_bk = batch['t_buckets'].to(device)
        attn_mask = batch['attn_mask'].to(device)
        labels = batch['labels'].to(device)
        ...
        out = mole_model(seq_ids=seq, attn_mask=attn_mask, t_buckets=t_bk, seqlen=seqlen)
        logits = _masked_item_logits(out.logits, item_token_ids)
        sums, bsz = recall_ndcg_sums(logits, labels, item_token_ids, topk=tuple(topk))
        for K in sums:
            sumd[K]['hit'] += sums[K]['hit']
            sumd[K]['dcg'] += sums[K]['dcg']
...
def encode_prompts_to_vecs_tisasrec(model: nn.Module, dataset: Dataset, collate_fn,
    ...
    """
    Encode prompt representations using TiSASRec hidden states.
    """
...
class RegionBankTiSAS:
    """
    Wrap RAIE logic for TiSASRec:
      - cluster original data to estimate vMF priors (mu/kappa/pi)
      - route finetune samples via update/expand/add
      - train region-specific LoRA adapters with original + boundary soft samples
      - route to region adapters at evaluation time
    """
    ...
    def _sync_priors_shape(self):
        ...
        if self.C0.shape[0] < K:
            # Fill C0 using new S rows, rather than the pre-expanded C.
            S_ex = self.S[self.C0.shape[0]:K, :]  # Shape: (K - C0, H)
            ...
            # Fallback: if some rows are zero, use the mean direction of current C.
    ...
    def map_finetune(self, finetune_ds: Dataset, pool_min=200, bic_gain=1e4):
        ...
        # Try creating new clusters (simplified).
    ...
    def route_and_eval(self, test_ds: Dataset, collate_fn, pad_id: int, item_token_ids: List[int],
        ...
        for k in sorted(idx_by_k.keys()):
            ...
            stat["cluster_id"] = int(k)
            stat["num_samples"] = len(sub)
...
def main():
    ...
    # Plugins (only replay/lwf used; ewc arguments kept for compatibility).
    ...
    ap.add_argument('--lora_target', type=str,
                    ...
                    help='Comma-separated module names (substring match), e.g. "Wq,Wk,Wv,Wo,ffn.0,ffn.3"')

    # ---- LSAT args ----
    ap.add_argument('--lsat_alpha', type=float, default=0.6, help='Long-term adapter weight for LSAT logit fusion')
    ap.add_argument('--lsat_long_epochs', type=int, default=1, help='Epochs for long-term adapter refinement')
    ap.add_argument('--lsat_short_epochs', type=int, default=3, help='Epochs for short-term adapter tuning')

    # ---- MoLE args ----
    ap.add_argument('--mole_num_experts', type=int, default=3, help='Number of LoRA experts for MoLE')
    ap.add_argument('--mole_gating_hidden', type=int, default=128, help='Hidden size of MoLE gating MLP')
    ap.add_argument('--mole_temp', type=float, default=0.7, help='Temperature for MoLE gating softmax')
    ap.add_argument('--mole_balance', type=float, default=0.01, help='KL regularization weight for balanced routing')

    # ===== RAIE plugin switches =====
    ap.add_argument('--raie_enable', action='store_true', default=True, help='Enable RAIE regional LoRA plugin')
    ...
    ap.add_argument('--raie_orig_mix_ratio', type=float, default=0.7)
    ...
    # Optional guardrail: allow full-parameter finetune to trigger Stage F (default False).
    ap.add_argument('--enable_full_finetune', action='store_true', default=False)

    args = ap.parse_args()

    # ---------- Mode-to-flag mapping (flow only, no logic changes) ----------
    # Disable EWC; RAIE is controlled by raie_enable.
    if args.mode == 'base':
        args.use_lora = False
        args.plugin = 'none'
        ...
        args.use_lora = True
        args.plugin = 'none'   # RAIE runs Stage R.
    ...
    if args.use_lora and (not PEFT_AVAILABLE):
        raise RuntimeError("peft is not available in the current environment.")
    ...
    original_test_path = os.path.join(args.data_dir, 'original.jsonl')
    ...
    original_test_ds = SeqDataset(original_test_path)
    ...
    original_test_loader = DataLoader(original_test_ds, batch_size=args.batch_size, shuffle=False, num_workers=2, collate_fn=collate)
    ...
    # Special mode: LSAT (long/short LoRA fusion).
    if args.mode == 'lsat':
        ...
        long_sched = torch.optim.lr_scheduler.LambdaLR(
            long_optim, lr_lambda=lambda step: min((step + 1) / max(1, long_warm), 1.0)
        )
        for ep in range(1, args.lsat_long_epochs + 1):
            tr = train_one_epoch(lora_model, train_loader, long_optim, long_sched, device, pad_id, item_token_ids, args.grad_clip)
            print(f"[LSAT][Long][Epoch {ep}] train_loss={tr:.4f}")

        FT_loader = DataLoader(finetune_ds, batch_size=args.finetune_batch_size, shuffle=True, num_workers=2, collate_fn=collate)
        if hasattr(lora_model, "set_adapter"):
            try:
        ...
        ft_item_ids = sorted({token2id.get(ex.target_token, None)
        ...
        E0 = lora_model.base_model.item_emb.weight.detach().clone() if hasattr(lora_model, "base_model") else lora_model.item_emb.weight.detach().clone()
        lambda_anchor = 1e-4
        ...
        for ep in range(1, args.lsat_short_epochs + 1):
            ft_loss = finetune_one_epoch(
                lora_model, FT_loader, ft_optim, ft_sched, device, pad_id, item_token_ids,
                args.grad_clip, plugin='none', replay_buf=None, ewc_state=None, teacher=None,
                replay_ratio=args.replay_ratio, lwf_T=args.lwf_T, lwf_alpha=args.lwf_alpha,
                E0=E0, ids_t=ids_t, lambda_anchor=lambda_anchor,
            )
            print(f"[LSAT][Short][Epoch {ep}] loss={ft_loss:.4f}")
        original_metrics = evaluate_lsat_tisas(
            lora_model, original_test_loader, device, pad_id, item_token_ids,
            long_adapter=long_adapter, short_adapter=short_adapter,
            alpha=args.lsat_alpha, topk=topk_tuple,
        )
        print("[original][lsat]", ' '.join([f"{k}:{v:.4f}" for k, v in sorted(original_metrics.items())]))
        metrics = evaluate_lsat_tisas(
            lora_model, test_loader, device, pad_id, item_token_ids,
            long_adapter=long_adapter, short_adapter=short_adapter,
            alpha=args.lsat_alpha, topk=topk_tuple,
        )
        print("[Test][lsat]", ' '.join([f"{k}:{v:.4f}" for k, v in sorted(metrics.items())]))
        with open(os.path.join(args.output_dir, 'test_metrics_lsat.json'), 'w', encoding='utf-8') as f:
    ...
    # Special mode: MoLE (multi-expert LoRA).
    if args.mode == 'mole':
        if finetune_ds is None or len(finetune_ds) == 0:
            raise RuntimeError('[MoLE] finetune.jsonl is required.')
        ...
        for i in range(num_exp):
            ...
            if name not in getattr(lora_model, 'peft_config', {}):
                lora_model.add_adapter(name, lora_cfg)
            if hasattr(lora_model, "set_adapter"):
                try:
        ...
        ft_optim = torch.optim.AdamW(params, lr=args.finetune_lr)
        ...
        for ep in range(1, args.finetune_epochs + 1):
            ft_loss = finetune_one_epoch_mole(
                mole_model, FT_loader, ft_optim, ft_sched, device,
                grad_clip=args.grad_clip, log_every=100,
            )
            print(f"[MoLE][Epoch {ep}] loss={ft_loss:.4f}")
        original_metrics = evaluate_mole_tisas(mole_model, original_test_loader, device, pad_id, item_token_ids, topk=topk_tuple)
        print("[original][mole]", ' '.join([f"{k}:{v:.4f}" for k, v in sorted(original_metrics.items())]))
        metrics = evaluate_mole_tisas(mole_model, test_loader, device, pad_id, item_token_ids, topk=topk_tuple)
        print("[Test][mole]", ' '.join([f"{k}:{v:.4f}" for k, v in sorted(metrics.items())]))
        with open(os.path.join(args.output_dir, 'test_metrics_mole.json'), 'w', encoding='utf-8') as f:
            json.dump(metrics, f, ensure_ascii=False, indent=2)

        mole_dir = os.path.join(args.output_dir, 'mole_adapters')
        os.makedirs(mole_dir, exist_ok=True)
        lora_model.save_pretrained(mole_dir, selected_adapters=adapter_names)
        torch.save({
            'gate_state': mole_model.gate.state_dict(),
            'adapter_names': adapter_names,
            'temperature': args.mole_temp,
            'balance_coef': args.mole_balance,
        }, os.path.join(mole_dir, 'mole_gate.pt'))
        return

    # ========== Stage F: conditional by mode ==========
    model = base_model
    did_f_stage = False

    # Base mode skips finetuning; other modes finetune when use_lora=True.
    if (args.mode != 'base') and args.use_lora and finetune_ds is not None and len(finetune_ds) > 0:
        # Collect item ids from finetune targets.
        ft_item_ids = sorted({
        ...
        # Snapshot item embeddings for anchoring.
        ...
        # Save default LoRA adapter (for RAIE or direct evaluation).
    ...
    # ===== Stage R: RAIE (raie mode only) =====
    if args.mode == 'raie':
        if not (args.use_lora and PEFT_AVAILABLE):
            raise RuntimeError("[RAIE] LoRA must be enabled and peft available.")
        if finetune_ds is None or len(finetune_ds) == 0:
            raise RuntimeError("[RAIE] finetune.jsonl is required.")
        if original_ds is None or len(original_ds) == 0:
            raise RuntimeError("[RAIE] original_stride1.jsonl is required (or provide matching data).")
        ...
        # Region routing evaluation (report weighted global metrics).
        ...
        topk_tuple = tuple(int(x) for x in args.topk.split(','))
        original_stats = bank.route_and_eval(original_test_ds, collate, pad_id, item_token_ids,
        ...
        if len(original_df) > 0:
            original_w = original_df["num_samples"].clip(lower=0)
            original_W = max(1, float(original_w.sum()))
            original_global_metrics = {m: float((original_df[m] * original_w).sum() / original_W) for m in original_df.columns if m.startswith(("Recall@", "NDCG@"))}
            print("[RAIE][Global]", original_global_metrics)
    else:
        if args.mode != 'base':
    ...
    # ----- Evaluation: final test by mode -----
    metrics = evaluate(model, test_loader, device, pad_id, item_token_ids, topk_tuple)
    ...
    # Save final model (with or without LoRA wrapper).
